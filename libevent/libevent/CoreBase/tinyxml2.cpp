...
// Bunch of unicode m_pCfgReaderInfo at:
                ...
                if ( (_flags & NEEDS_NEWLINE_NORMALIZATION) && *p == CR ) {
                    ...
                    *q++ = LF;
                    ...
                    if ( *(p+1) == '#' ) {
                        ...
                        int index=0;
                        for(; index<NUM_ENTITIES; ++index ) {
                            if (    strncmp( p+1, entities[index].pattern, entities[index].length ) == 0
                                    && *(p+entities[index].length+1) == ';' ) {
                                // Found an entity convert;
                                *q = entities[index].value;
                                ++q;
                                p += entities[index].length + 2;
                                break;
                            }
                        }
...
{
    for( int index=0; index<ENTITY_RANGE; ++index ) {
        _entityFlag[index] = false;
        _restrictedEntityFlag[index] = false;
    }
    for( int index=0; index<NUM_ENTITIES; ++index ) {
        TIXMLASSERT( entities[index].value < ENTITY_RANGE );
        if ( entities[index].value < ENTITY_RANGE ) {
            _entityFlag[ (int)entities[index].value ] = true;
        }
    }
        ...
        while ( *q ) {
            ...
            if ( *q > 0 && *q < ENTITY_RANGE ) {
                ...
                if ( flag[(unsigned)(*q)] ) {
                    ...
                    for( int index=0; index<NUM_ENTITIES; ++index ) {
                        if ( entities[index].value == *q ) {
                            Print( "&%s;", entities[index].pattern );
                            break;
                        }
                    }
                    ++p;
